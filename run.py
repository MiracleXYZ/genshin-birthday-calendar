...
async def main():
    ...
    calendar = Calendar(
        ...
        version="2.0",
        extras=[
            {"name": "name", "value": "原神生日日历"},
            {"name": "x-wr-calname", "value": "原神生日日历"},
            {"name": "timezone-id", "value": "Asia/Shanghai"},
            {"name": "x-wr-timezone", "value": "Asia/Shanghai"},
            {"name": "X-PUBLISHED-TTL", "value": "PT1H"},
            {"name": "x-wr-caldesc", "value": "提瓦特全角色生日日历"},
            {"name": "REFRESH-INTERVAL;VALUE=DURATION", "value": "PT1H"},
        ],
    )
    if (
    ...
    for data in [
        ...
        if v["birthday"][0] != 0 and not v.get("beta", False)
    ]:
